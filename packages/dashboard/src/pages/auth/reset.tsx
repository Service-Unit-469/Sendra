...
export default function Index() {
  ...
  const resetPassword = async (data: { password: string }) => {
    setSubmitted(true);
    await network.fetch("/auth/reset", {
      method: "POST",
      body: {
          ...
          <div>
            ...
            <div className="relative mt-1">
              ...
              <div className="absolute inset-y-0 right-0 flex cursor-pointer items-center pr-3">
                <svg onClick={() => setHidePassword(!hidePassword)} className="h-5 w-5 text-neutral-400" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" aria-hidden="true">
                  {hidePassword ? (
                    <>
                      <path
                        fillRule="evenodd"
