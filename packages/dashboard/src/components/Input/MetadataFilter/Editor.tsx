...
export default function MetadataFilterEditor({ onChange, contacts }: { onChange: (metadataFilter: MetadataFilterGroupType) => void, contacts: Contact[] }) {
    ...
    return (<>
        ...
        {group.filters.map((filter, index) => (
            <div className={"sm:col-span-4"}>
                <Filter key={index} initialFilter={filter} onFilterChange={(filter) => setGroup({ ...group, filters: group.filters.map((f, i) => i === index ? filter : f) })} contacts={contacts} index={index} onFilterRemove={(index) => setGroup({ ...group, filters: group.filters.filter((_, i) => i !== index) })} />
            </div>
        ))}
