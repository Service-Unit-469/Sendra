 ...
 */
export default function Filter({ initialFilter, onFilterChange, contacts, index, onFilterRemove }: { initialFilter: MetadataFilterType, onFilterChange: (filter: MetadataFilterType) => void, contacts: Contact[], index: number, onFilterRemove: (index: number) => void }) {
    ...
    const values = useMemo(() => {
        ...
        if (filter.field === 'email') {
            ...
            contacts.forEach((c) => {
                const value = c.data[filter.field as string];
